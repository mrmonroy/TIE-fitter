...
############################
outdir = os.path.join(basedir,'mark_omega/std_maps/weights_validation/postunblinding/')
...
for config in configs:
	...
	for ibin in range(len(binedges)-1):
		...
		if config=='no_weights':
			wtheta_dict['{0}_theta'.format(ibin)] = theta
		else:
			assert (theta==wtheta_dict['{0}_theta'.format(ibin)]).all()
		
		wtheta_dict['{0}_{1}_wtheta'.format(ibin,config)] = wtheta_pix
	
np.save(os.path.join(outdir,'wtheta_dict_{0}_binslop{1}_nside{2}_methods_comparison_postunblinding.npy'.format(sample_version,bin_slop,nside)),wtheta_dict)
