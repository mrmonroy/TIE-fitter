...
def main():
	...
	mockdir = os.path.join(basedir,'lognormal_mocks/output_{0}/maps/'.format(nside))
	mockfile = 'y6_maglim_uncon_nside512_maskv4_nside{NSIDE}_mock{MOCK_NUMBER}.fits.gz'.format(NSIDE=nside,MOCK_NUMBER=mocknumber)
	
	wmapdir = os.path.join(basedir,'mark_omega/std_maps/weights_validation/over-correction_bias/mock_wmaps_std{0}/'.format(num_maps))
	wmapfile = 'w_map_mock{0}_nside{1}_mark_omega_std{2}.fits.gz'.format(mocknumber,nside,num_maps)
	
	
	outdir = os.path.join(basedir,'mark_omega/std_maps/weights_validation/over-correction_bias/mock_wtheta_std{0}/'.format(num_maps))
	...
	if do_weight == True:
		...
		np.save(os.path.join(outdir,'w_pix_weights_uncontaminated_nside{nside}_mock{MOCK_NUMBER}.npy'.format(nside=nside,MOCK_NUMBER=mocknumber)), w_pix_dict)
	...
	if do_noweight == True:
		...
		np.save(os.path.join(outdir,'w_pix_noweights_uncontaminated_nside{nside}_mock{MOCK_NUMBER}.npy'.format(nside=nside,MOCK_NUMBER=mocknumber)), w_pix_noweight_dict)
