...
outdir = os.path.join(basedir,'mark_omega/std_maps/weights_validation/over-correction_bias/mock_wmaps_std{0}/'.format(num_maps))
...
for ibin in zbins:
	...
	mock_i_coeffs = np.array(mock_coeff_dict[(int(mocknumber),ibin)][0])
	...
	for i,imap in enumerate(maplist):
		...
		pmask = (bin_centers_>-1.*np.abs(mock_i_coeff_))*(bin_centers_<np.abs(mock_i_coeff_))
		...
		if save_plots:
			plt.axvline(x=mock_i_coeff_,ls='--',color='b',label='a data')
			plt.axvline(x=-mock_i_coeff_,ls='--',color='r',label='-a data')
		...
		fs_ibin[~maskup.mask] = fs_ibin[~maskup.mask]+mock_i_coeff_*(1.-pval_)*std_data_up_
		print(imap,mock_i_coeff_,(1.-pval_))
		print('Coeff. a_{0} = {1}'.format(i,mock_i_coeff_))
	
	mock_i_coeff0 = mock_coeff_dict[(int(mocknumber),ibin)][1]
	print(mock_i_coeff0)
	mock_coeffs0 = []
	for imock in range(nmocks):
	...
	print('Coeff b = ',mock_i_coeff0)
	fs_ibin[~maskup.mask] = fs_ibin[~maskup.mask]+mock_i_coeff0
...
lsssys.save_mock_format(os.path.join(outdir,'w_map_mock{0}_nside{1}_mark_omega_std{2}.fits'.format(mocknumber,upnside,num_maps)),zbins,wmaplist,clobber=True)
