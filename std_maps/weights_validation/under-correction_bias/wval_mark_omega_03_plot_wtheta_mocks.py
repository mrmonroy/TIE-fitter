...
wthetadirc = os.path.join(basedir,'isd/weights_validation/wtheta_binslop{0}_nside{1}_treecorr336_{2}_pixel_contaminated_{3}/'.format(bin_slop,nside,corrlabel,clabel))
wthetadir = os.path.join(basedir,'mark_omega/std_maps/weights_validation/under-correction_bias/mock_wtheta_std{0}/'.format(num_maps))
outdir = os.path.join(basedir,'mark_omega/std_maps/weights_validation/under-correction_bias/plots_wtheta_std{0}/'.format(num_maps))
if os.path.exists(outdir)==False:
	os.mkdir(outdir)

wlabel_mock0 = 'w_pix_uncontaminated_nside{nside}_mock{MOCK_NUMBER}.npy'
wlabel_cmock = 'w_pix_contaminated_nside{nside}_mock{MOCK_NUMBER}.npy'
wlabel_wmock = 'w_pix_decontaminated_nside{nside}_mock{MOCK_NUMBER}.npy'
...
for ibin in zbins:
	...
	for i in range(nmocks):
		...
		w_cmock = np.load(os.path.join(wthetadirc,wlabel_cmock.format(nside=nside,MOCK_NUMBER=i)),allow_pickle=True).ravel()[0]
		assert (w_cmock['theta']==w_ref['theta']).all()
		
		wtheta_wmock_list.append(w_wmock[ibin,ibin])
		wtheta_cmock_list.append(w_cmock[ibin,ibin])
	
	wmean_mock0 = np.mean(wtheta_mock0_list,axis=0)
	#print wmean_mock0
	wmean_wmock = np.mean(wtheta_wmock_list,axis=0)
	wmean_cmock = np.mean(wtheta_cmock_list,axis=0)
	...
	plt.plot(theta,theta*wmean_cmock,'-',color='r',label='Contaminated mocks'.format(clabel))
	plt.plot(theta,theta*wmean_wmock,'-',color='b',label='Mark Omega Decont. mocks, std{0}'.format(num_maps))
	plt.errorbar(theta,theta*wmean_mock0,yerr=theta*errbar,color='k',ecolor='k',label='Uncontaminated mocks')
	...
	figname = 'wtheta_test4_zbin{0}_mark_omega_std{1}_nmocks{2}.png'.format(ibin,num_maps,nmocks)
