...
####################
mocklist = [mock for mock in os.listdir(mockdir) if '.fits.gz' in mock and str(nside) in mock]
...
if __name__=='__main__':
	...
	if test==False:
		np.save(os.path.join(outdir,'mock_coeff_dict_nside{0}_std{1}.npy'.format(nside,len(maplist))),coeff_dict)
	else:
		np.save(os.path.join(outdir,'mock_coeff_dict_nside{0}_std{1}_TEST.npy'.format(nside,len(maplist))),coeff_dict)
